...
import { InfraModel, Component, DependencyRelationship, StructuralRelationship } from '@aws-c2a/models';
...
const generateClusterChildren = (
  ...
  if(!nodes.has(component)){
    ...
    if(structuralChildren.length > 0) {
      ...
      component.outgoing.forEach(relationship =>
        generateClusterChildren(graph, relationship.target, nodes, clusterIds));
    } else {
