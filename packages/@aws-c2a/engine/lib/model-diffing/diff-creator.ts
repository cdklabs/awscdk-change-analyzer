...
export class DiffCreator {
  ...
  private diffComponents(oldComponents: Component[], newComponents: Component[]): ComponentOperation[]{
    ...
    const renames = renamedMatches.matches.map(({transition}) =>
      new RenameComponentOperation({}, {componentTransition: transition}));
    const removals = renamedMatches.unmatchedA.map(c =>
      new RemoveComponentOperation({}, {componentTransition: this.createComponentTransition({v1: c})}));
    const insertions = renamedMatches.unmatchedB.map(c =>
      new InsertComponentOperation({}, {componentTransition: this.createComponentTransition({v2: c})}));
