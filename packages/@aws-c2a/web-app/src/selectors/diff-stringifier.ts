...
export class DiffStringifier<T> {
  ...
  constructor(
    ...
    private readonly changesResolver: ChangesResolver<T> =
  ...
  private buildArray(
    ...
    const resultingStrings = entries.map(({obj, path: entryPath, highlights = {}}): DiffStringChunk<T>[] => [
      indentationStr,
      ...this.changesResolver(obj, entryPath).structures
        .flatMap(({content, highlights: resolvedHighlights}) =>
          this.buildDiffString(
            content,
            newIndentation,
            entryPath,
            this.joinHighlights([activeHighlights, highlights, resolvedHighlights]),
          ),
        ),
  ...
  private buildObject(
    ...
    const resultingStrings = entries.map(({obj, path: entryPath, highlights = {}}): DiffStringChunk<T>[] => {
      ...
      return [
        ...(resolvedChanges.renamedFrom !== undefined)
          ? [{
            str: resolvedChanges.renamedFrom.toString(),
            highlights: this.addHighlights(activeHighlights, [DiffHighlightType.Remove]),
            path: entryPath,
          }]
          : [],
        {
          str: `${this.indent(newIndentation)}${entryPath.slice(-1)}: `,
          highlights: this.addHighlights(activeHighlights, (resolvedChanges.renamedFrom !== undefined)
            ? [DiffHighlightType.Insert]
            : []),
          path: entryPath,
        },
