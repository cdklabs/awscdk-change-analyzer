...
export abstract class CFEntity {
    ...
    constructor(name: string, definition: Record<string, any>, args: CFParserArgs, rootComponent: Component){
        ...
        this.dependencyRefs = new Map(Object.entries(CFEntity.readRefsInExpression(definition)))
    ...
    createRelationshipsAndComponents(cfEntities: Record<string, CFEntity>): [Relationship[], Component[]]{
        ...
        const relationships = [
            ...Array.from(this.dependencyRefs)
                .flatMap(([type, refs]) =>
                    refs.map(ref =>
                        this.createDependencyRelationship(cfEntities[ref].component, type)
                )),
            rootRelationship
        ]
    ...
    static readRefsInExpression = (expression: any, refPath?: string): Record<string, string[]> => {
        if(typeof(expression) !== 'object' || expression == null)
            ...
            return Object.entries(expression).reduce((acc, [k, v]) => {
                ...
                let refs
                return {...acc,
                    ...CFEntity.readRefsInPropertyMapping[k] && (refs = CFEntity.readRefsInPropertyMapping[k](v))
                        ? {[newRefPath]: refs}
