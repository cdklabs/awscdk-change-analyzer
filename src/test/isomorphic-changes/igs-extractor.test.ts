...
test('Extract IG of operation type, component type and component subtype', () => {
    
    const diff = createTestCase1();
    const groups = extractComponentOperationsIGs(diff);
    console.log(diff.componentOperations);
    console.log(groups);
});
...
test('Group operations from big template', () => {
    ...
    const isomorphicGroups = extractComponentOperationsIGs(diff);
});